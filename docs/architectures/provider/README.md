...
provider adapter 는 provider identity 와 public capability field 로 구성된다. adapter 는 provider client 를 받아 `mwosa` 가 이해하는 역할 interface 구현체로 감싼다.
...
이 provider 는 `DailyBars` 와 `Instruments` capability 를 제공한다. quote snapshot 을 제공하지 않는 provider 라면 `Quotes` field 를 가지지 않는다.
...
type Provider struct {
	...
	Quotes      quote.Snapshotter
	DailyBars   dailybar.Fetcher
	Instruments instrument.Searcher
}
```

public field 만 봐도 provider 가 어떤 capability 를 제공하는지 알 수 있다. 지원하지 않는 기능은 unsupported method 로 남기지 않고 field 자체를 두지 않는다.
...
provider 별 구현체는 role interface 를 public capability field 로 가진다.
...
`datago.Provider` 자체가 `dailybar.Fetcher` 와 `instrument.Searcher` 를 만족할 필요는 없다. service 는 provider router 를 통해 route 된 capability field 를 받는다.

## Profile

public capability field 는 provider 가 코드상으로 어떤 역할을 제공하는지 보여준다. 실제 provider 선택에는 실행 시점의 조건이 필요하다. 이 조건은 각 role 의 `Profile` 이 제공한다.
...
registry 는 public capability field 에서 role interface 와 profile 을 수집한다. provider router 는 registry 의 후보 목록을 사용해 요청에 맞는 provider role 로 라우팅한다. 이때 provider 이름만 비교하지 않고 group, operation, auth scope 까지 함께 본다.
...
registry 는 provider instance 의 exported field 를 검사하면서 `provider.RoleProvider` 를 만족하는 capability 를 수집한다.

```go
func (r *Registry) RegisterProvider(p provider.IdentityProvider) error {
	for _, field := range exportedFields(p) {
...
- provider struct 의 public capability field 는 지원 역할을 보여준다.
