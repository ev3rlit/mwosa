...
mwosa get daily 491820 --prefer-provider data-go-etf
```
...
CLI 에서 사용하는 provider 이름은 짧은 lowercase kebab-case 를 기본으로 한다.

| 예시 | 설명 |
| --- | --- |
| `kis` | 한국투자증권 KIS Developers |
| `data-go-etf` | 공공데이터포털 금융위원회 증권상품시세정보 |
...
| `data-go-etf` | `core` | 한국 ETF, ETN, ELW | `candles`, `instrument` | ETF/ETN 일별 시세와 종목 메타데이터 보강용이다. 실시간 quote provider 로 보지 않는다. |
...
| `candles` | `kis`, `data-go-etf` | `krx`, `yahoo-finance`, `alpha-vantage`, `polygon`, `stooq`, `eodhd` |
| `instrument` | `data-go-etf`, `krx`, `kis` | `openfigi`, `yahoo-finance`, `fmp` |
...
- `docs/providers/data-go-etf/README.md`
