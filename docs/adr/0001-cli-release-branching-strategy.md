...
다만 작업 브랜치를 원격에 올릴 수 있어야 PR, CI, 리뷰, 작업 공유를 사용할 수 있다. 따라서 `feat/*`, `fix/*`, `docs/*`, `exp/*` 는 원격 브랜치로 허용한다.
...
- `feat/*`, `fix/*`, `docs/*`, `exp/*` 는 원격에 push 할 수 있는 작업 브랜치로 둔다.
- 리모트 브랜치는 `main`, `release/*`, `feat/*`, `fix/*`, `docs/*`, `exp/*` 로 제한한다.
...
이 결정으로 작업 흐름은 단순하게 유지된다. 작은 `feat/*`, `fix/*`, `docs/*`, `exp/*` 브랜치에서 기능, 수정, 문서, 실험을 나눠 진행하고, 검증된 결과만 `main` 으로 통합한다.
...
`feat/*`, `fix/*`, `docs/*`, `exp/*` 를 로컬 전용으로만 두는 방식이다. 원격 브랜치가 적어지는 장점은 있지만 PR, CI, ruleset 기반 merge gate 를 쓰기 어렵다. 현재는 주요 작업 브랜치를 원격에 허용하고, 이름 allowlist 로 불필요한 접두사를 막는 쪽을 선택한다.
