...
여러 종목을 한 번에 볼 때는 공백으로 symbol 을 나열한다. 다른 명령의 출력이나 AI 에이전트가 만든 symbol 목록을 넘길 때는 `--stdin` 을 사용한다.
...
| 명령어                         | 설명                                                           |
| ------------------------------ | -------------------------------------------------------------- |
| `mwosa build context <target>` | AI 에이전트가 리서치에 바로 사용할 수 있는 데이터 묶음을 만든다. |
