...
func (c *Client) GetItemInfo(ctx context.Context, query Query) (ItemInfoResult, error) {
	result, err := fetchPage[ListedItem](c, ctx, OperationGetItemInfo, query.values(), query.pageNo(), query.numOfRows())
	if err != nil {
		return ItemInfoResult{}, err
	}
	return ItemInfoResult{Items: result.Items, NumOfRows: result.NumOfRows, PageNo: result.PageNo, TotalCount: result.TotalCount}, nil
}

func (c *Client) GetAllItemInfo(ctx context.Context, query Query) (ItemInfoResult, error) {
	query = query.forAllPages()
	result, err := fetchAllPages[ListedItem](c, ctx, OperationGetItemInfo, query.values(), query.numOfRows())
	if err != nil {
		return ItemInfoResult{}, err
	}
	return ItemInfoResult{Items: result.Items, NumOfRows: result.NumOfRows, PageNo: result.PageNo, TotalCount: result.TotalCount}, nil
...
func fetchAllPages[T any](c *Client, ctx context.Context, operation string, params url.Values, numOfRows int) (result[T], error) {
	...
	if err != nil {
		return result[T]{}, errb.With("page", 1).Wrapf(err, "fetch datago krx listed first page")
	}
	...
	for pageNo := 2; pageNo <= pageCount; pageNo++ {
		...
		if err != nil {
			return result[T]{}, errb.With("page", pageNo).Wrapf(err, "fetch datago krx listed page")
		}
...
func fetchPageOnce[T any](c *Client, ctx context.Context, operation string, params url.Values, pageNo int, numOfRows int) (apiResponse[T], bool, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return apiResponse[T]{}, shouldRetryStatus(resp.StatusCode), errb.With("status", resp.StatusCode, "body", bodyText).Errorf("datago remote error provider=%s group=%s operation=%s page=%d status=%d body=%s", ProviderDataGo, GroupKRXListedInfo, operation, pageNo, resp.StatusCode, bodyText)
	}
	...
	if decoded.Header.ResultCode != "" && decoded.Header.ResultCode != "00" {
		return apiResponse[T]{}, false, errb.With("result_code", decoded.Header.ResultCode, "result_msg", decoded.Header.ResultMsg).Errorf("datago remote error provider=%s group=%s operation=%s page=%d result_code=%s result_msg=%s", ProviderDataGo, GroupKRXListedInfo, operation, pageNo, decoded.Header.ResultCode, decoded.Header.ResultMsg)
	}
