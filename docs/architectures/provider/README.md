...
`mwosa` 는 KIS, 공공데이터포털, KRX 같은 여러 주식 API provider 를 같은 CLI 명령 체계로 조회한다. 각 provider 는 지원하는 시장, 자산군, endpoint, freshness 가 다르다. provider 아키텍처는 이 차이를 provider adapter 에서 처리하고, service layer 는 일관된 역할 interface 와 canonical data 만 사용하게 한다.
...
| provider adapter | provider implementation 을 `mwosa` 내부 role interface 와 canonical data 로 연결하는 adapter 다. CLI config 변환, request/result 변환, error 변환, normalization 연결을 담당한다. |
| provider bridge | provider adapter 역할을 설명할 때 쓰는 보조 표현이다. package, folder, type 이름의 접미사로는 쓰지 않는다. |
...
짧게 말하면, provider implementation 은 외부 API 를 호출하고, provider adapter 는 이를 `mwosa` 언어로 번역하며, service/domain layer 는 실제 앱 동작과 도메인 계산을 수행한다.
...
provider 는 하나의 거대한 객체가 아니라 여러 역할을 가진 객체다. 아래 흐름에서 `provider adapter` 는 CLI 안 연결 지점이고, `external provider package` 는 provider implementation 이다.
...
provider adapter 는 provider identity 와 embedded role 로 구성된다.
...
provider 역할 interface, registry, router 는 `providers/core` 아래에 둔다. provider 별 구현체도 같은 `providers` 아래에 두고, 폴더 이름은 provider 이름을 그대로 쓴다.
...
이전처럼 `provider/` 와 `providers/` 를 나누거나, `kisbridge`, `datagobridge` 처럼 `bridge` 접미사를 붙이지 않는다. 특정 provider 폴더가 이미 bridge 역할을 한다는 것을 경로와 아키텍처 문맥으로 알 수 있기 때문이다.
...
provider 별 구현체는 이 role struct 를 embed 한다.
...
Go 의 method promotion 때문에 `datago.Provider` 는 자연스럽게 `dailybar.Fetcher` 와 `instrument.Searcher` 를 만족한다.
...
- market, security type, group, operation, freshness, auth, priority 를 role profile 과 비교한다.
...
service 는 router 와 role interface 에만 의존한다. provider 별 구현 package 이름이나 external provider package 타입은 service 에 전달하지 않는다.

## Provider Adapter

provider adapter 는 external provider package 와 `mwosa` 내부 role interface 사이를 연결한다. 문서에서는 이 역할을 provider bridge 라고 부를 수 있지만, 폴더 이름에는 `bridge` 접미사를 붙이지 않는다.
...
provider 가 REST API 를 쓰는지, SDK 를 쓰는지, 파일을 읽는지는 provider 별 구현체 또는 external provider package 안에서만 결정한다. service 와 provider role interface 에는 특정 HTTP client library type 을 노출하지 않는다.
...
func New(client *datago.Client) *Provider {
	return &Provider{
		...
		Fetch: dailybar.NewFetch(dailybar.Profile{
			...
			Group:         provider.GroupID("securitiesProductPrice"),
			Operations: []provider.OperationID{
				"getETFPriceInfo",
				"getETNPriceInfo",
				"getELWPriceInfo",
...
provider group 은 provider router 가 실제 후보를 고를 때 보는 하위 단위다. 사용자는 보통 `--provider datago` 처럼 provider 만 지정하지만, 내부에서는 어떤 group 과 operation 이 그 요청을 처리하는지 기록해야 한다.
...
- provenance 에 provider 이름만 남기면 어떤 원천 API 에서 온 데이터인지 부족하다.
